...
async function testTrigger() {
    try {
        ...
        console.error('âŒ FAILURE: Nairobi hospital was deleted! Protection trigger failed.');
        process.exit(1);
    } catch (err) {
        if (err.message && err.message.includes('DATABASE SAFETY VIOLATION')) {
            console.log('âœ… SUCCESS: Protection trigger correctly BLOCKED deletion of real organization:');
            ...
            console.log('âœ… SUCCESS: Test fixture with "Test" prefix was successfully deleted as expected.');
